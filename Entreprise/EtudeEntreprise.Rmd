...
nature_contrat_filtered <- subset(merged_data, 
...
anova_localisation_emploi <- aov(remuneration_prime ~ localisation_emploi, data = merged_data)
...
```{r}
data_no_na <- na.omit(merged_data[, c("remuneration_prime", "entreprise_groupe")])
...
merged_data$taille_entreprise <- recode(merged_data$taille_entreprise,
...
anova_taille_entreprise <- aov(remuneration_prime ~ taille_entreprise, data = merged_data)
...
anova_region_taille <- aov(remuneration_prime ~ region_emploi * taille_entreprise, data = merged_data)
...
```{r}
# ANOVA pour évaluer l'impact de la région de l'emploi et de la taille de l'entreprise sur la rémunération prime
anova_region_sexe <- aov(remuneration_prime ~ region_emploi * sexe, data = merged_data)
...
valeurs_regions <- unique(merged_data$region_emploi)

options(repr.plot.width = 10, repr.plot.height = 6)
# Créer un graphique d'interaction en boîte avec facettes
interaction_plot <- ggplot(merged_data, aes(x = sexe, y = remuneration_prime/10000, fill = sexe)) +
  geom_boxplot() +
...
```{r}
# ANOVA pour évaluer l'impact de la région de l'emploi et de la taille de l'entreprise sur la rémunération prime
anova_region_filiere <- aov(remuneration_prime~ region_emploi * filiere, data = merged_data)
...
```{r}
#stockage des différentes région : 
valeurs_regions <- unique(merged_data$filiere)

# Créer un graphique d'interaction en boîte avec facettes
interaction_plot <- ggplot(merged_data, aes(x = filiere, y = remuneration_prime/10000, fill = filiere)) +
  geom_boxplot() +
  facet_wrap(~region_emploi, scales = "free_y") + 
  labs(title = "Interaction entre region_emploi et sexe",
       x = "filiere",
