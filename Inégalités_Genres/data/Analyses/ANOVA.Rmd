...
##Import des données 
```{r}
donnees <- read.csv("../donnees_combinees_filtrees.csv")
```
...
```{r}
resultat_anova <- aov(remuneration_annuelle_brute ~ sexe, data = donnees)
...
resultat_anova_filiere <- aov(remuneration_annuelle_brute ~ filiere, data = donnees)
...
donnees_filtrees_sans_vide_nationalite_francaise <- donnees[donnees$nationalite_francaise != " ", ]
...
donnees_sansVide_nationalite <- donnees
...
donnees_filtrees_sans_vide_nationalite <- donnees[donnees$nationalite != "", ]
...
unique(donnees$prime)

```

```{r}
# Exclure les observations avec des valeurs vides dans la variable "prime"
donnees_filtrees_sans_vide_prime <- donnees[donnees$prime != " ", ]
...
modele_regression <- lm(remuneration_annuelle_brute ~ sexe * filiere, data = donnees)
...
modele_anova <- aov(remuneration_annuelle_brute ~ sexe * filiere, data = donnees)
...
donnees$nationalite <- as.factor(donnees$nationalite)

# Réalisation de l'ANOVA à deux facteurs
modele_anova <- aov(remuneration_annuelle_brute ~ sexe * nationalite, data = donnees)
