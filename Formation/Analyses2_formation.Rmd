...
```{r}
library(ggplot2)
histogram4 <- ggplot(data, aes(x = filiere, y = remuneration_prime, fill = filiere)) +
...
filtered_data <- data[!is.na(data$remuneration_prime) & data$type_combined != "" & data$remuneration_prime != "", ]
...
```{r}
boxplot(remuneration_prime ~ filiere_combined2, data = filtered_data2)

boxplot(remuneration_prime ~ type_combined, data = filtered_data)

# Convertir date_diplome en caractère pour la comparaison
data$date_diplome <- as.character(data$date_diplome)

# Filtrer les données en excluant les observations où date_diplome est égale à "721"
filtered_data_boxplot <- data[data$date_diplome != "721", ]

# Créer le boxplot avec les données filtrées
boxplot(remuneration_prime ~ date_diplome, data = filtered_data_boxplot)
...
moyennes_par_groupe <- data %>%
  group_by(filiere, type_formation, date_diplome) %>%
  summarise(
...
```{r}
 ggplot(filtered_data2, aes(x = factor(filiere_combined2), y = remuneration_prime, fill = factor(sexe))) +
      geom_boxplot(width = 0.6) + 
      labs(x = "Filière", y = "Rémunération Prime") +
      ggtitle(paste("Variation de la rémunération Prime par filière et sexe")) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 50, hjust = 1))
```

```{r}
data$type_combined2[data$type_formation %in% c("Sous contrat d apprentissage", "Sous contrat d'apprentissage")] <- "Sous contrat d'apprentissage"


ggplot(filtered_data, aes(x = factor(type_combined), y = remuneration_prime, fill = factor(sexe))) +
      geom_boxplot(width = 0.6) +  
      labs(x = "Type de Formation", y = "Rémunération Prime") +
      ggtitle(paste("Variation de la rémunération Prime par type de formation et sexe")) +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 50, hjust = 1))
```
...
```{r}
 ggplot(subset(data, date_diplome != "721"), aes(x = factor(date_diplome), y = remuneration_prime, fill = factor(sexe))) +
      geom_boxplot(width = 0.6) +  
